...
define(["require", "exports", "assert", "vs/base/common/winjs.base", "vs/workbench/common/editor", "vs/workbench/common/editor/diffEditorInput", "vs/base/common/uri", "vs/workbench/services/untitled/common/untitledEditorService", "vs/workbench/test/workbenchTestServices"], function (require, exports, assert, winjs_base_1, editor_1, diffEditorInput_1, uri_1, untitledEditorService_1, workbenchTestServices_1) {
    'use strict';
    ...
    suite('Workbench - Editor', function () {
        ...
        test('toResource', function () {
            ...
            assert.equal(editor_1.toResource(untitled, { filter: 'untitled' }).toString(), untitled.getResource().toString());
            assert.equal(editor_1.toResource(untitled, { filter: ['file', 'untitled'] }).toString(), untitled.getResource().toString());
            assert.ok(!editor_1.toResource(untitled, { filter: 'file' }));
            ...
            assert.equal(editor_1.toResource(file, { filter: 'file' }).toString(), file.getResource().toString());
            assert.equal(editor_1.toResource(file, { filter: ['file', 'untitled'] }).toString(), file.getResource().toString());
            assert.ok(!editor_1.toResource(file, { filter: 'untitled' }));
            ...
            assert.ok(!editor_1.toResource(diffEditorInput, { filter: 'file' }));
            assert.ok(!editor_1.toResource(diffEditorInput, { supportSideBySide: false }));
            assert.equal(editor_1.toResource(file, { supportSideBySide: true }).toString(), file.getResource().toString());
            assert.equal(editor_1.toResource(file, { supportSideBySide: true, filter: 'file' }).toString(), file.getResource().toString());
            assert.equal(editor_1.toResource(file, { supportSideBySide: true, filter: ['file', 'untitled'] }).toString(), file.getResource().toString());
        });
    });
});
