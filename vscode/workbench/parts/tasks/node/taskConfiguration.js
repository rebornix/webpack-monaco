...
define(["require", "exports", "crypto", "vs/nls", "vs/base/common/objects", "vs/base/common/platform", "vs/base/common/types", "vs/base/common/uuid", "vs/base/common/parsers", "vs/platform/markers/common/problemMatcher", "../common/tasks", "../common/taskDefinitionRegistry"], function (require, exports, crypto, nls, Objects, Platform, Types, UUID, parsers_1, problemMatcher_1, Tasks, taskDefinitionRegistry_1) {
    /*---------------------------------------------------------------------------------------------
    ...
    (function (CommandOptions) {
        ...
        var defaults = { cwd: '${workspaceRoot}' };
    ...
    (function (CommandConfiguration) {
        ...
        function onlyTerminalBehaviour(value) {
            return value &&
        ...
        CommandConfiguration.onlyTerminalBehaviour = onlyTerminalBehaviour;
        function assignProperties(target, source) {
            if (isEmpty(source)) {
                return target;
            }
    ...
    (function (ConfiguringTask) {
        ...
        function from(external, context, index) {
            ...
            if (Types.isString(customize)) {
                ...
                Object.keys(typeDeclaration.properties).forEach(function (property) {
    ...
    (function (CustomTask) {
        function from(external, context, index) {
            ...
            var result = {
                ...
                _source: Objects.assign({}, source, { config: { index: index, element: external, file: '.vscode\\tasks.json' } }),
                _label: taskName,
                name: taskName,
                identifier: taskName,
                command: undefined
    ...
    (function (TaskParser) {
                    ...
                    if (customTask) {
                        ...
                        if (context.engine === Tasks.ExecutionEngine.Terminal && customTask.command && customTask.command.name && customTask.command.runtime === Tasks.RuntimeType.Shell && customTask.command.args && customTask.command.args.length > 0) {
                            if (hasUnescapedSpaces(customTask.command.name) || customTask.command.args.some(hasUnescapedSpaces)) {
                                context.problemReporter.warn(nls.localize('taskConfiguration.shellArgs', 'Warning: the task \'{0}\' is a shell command and either the command name or one of its arguments has unescaped spaces. To ensure correct command line quoting please merge args into the command.', customTask.name));
                            }
                        }
                        if (schema2_0_0) {
        ...
        function quickParse(externals, context) {
            if (!externals) {
                return undefined;
            }
            var result = [];
            for (var index = 0; index < externals.length; index++) {
                var external_2 = externals[index];
                if (isCustomTask(external_2)) {
                    result.push(CustomTask.from(external_2, context, index));
                }
                else {
                    result.push(ConfiguringTask.from(external_2, context, index));
                }
            }
            return result;
        }
        TaskParser.quickParse = quickParse;
    ...
    var NullProblemReporter = (function (_super) {
        __extends(NullProblemReporter, _super);
        function NullProblemReporter() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        NullProblemReporter.prototype.clearOutput = function () { };
        ;
        return NullProblemReporter;
    }(parsers_1.NullProblemReporter));
    ...
    var ConfigurationParser = (function () {
        function ConfigurationParser(problemReporter, uuidMap) {
    ...
    function findTaskIndex(fileConfig, task) {
        ...
        var localMap = new UUIDMap(uuidMap);
        var context = {
            problemReporter: this.problemReporter,
            uuidMap: localMap,
            namedProblemMatchers: undefined,
        ...
        try {
            localMap.start();
            var tasks = TaskParser.quickParse(fileConfig.tasks, context);
    ...
    exports.findTaskIndex = findTaskIndex;
});
/*
