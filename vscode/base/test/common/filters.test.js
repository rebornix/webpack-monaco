define(["require", "exports", "assert", "vs/base/common/filters"], function (require, exports, assert, filters_1) {
    ...
    suite('Filters', function () {
        ...
        test('topScore - fuzzyScore', function () {
            ...
            assertTopScore(filters_1.fuzzyScore, 'p', 0, 'parse', 'posix', 'pafdsa', 'path', 'p');
            assertTopScore(filters_1.fuzzyScore, 'pa', 0, 'parse', 'pafdsa', 'path');
            // issue #14583
            assertTopScore(filters_1.fuzzyScore, 'log', 3, 'HTMLOptGroupElement', 'ScrollLogicalPosition', 'SVGFEMorphologyElement', 'log');
        ...
        test('nextTypoPermutation', function () {
            function assertTypos(pattern) {
                ...
                for (var _a = 0, variants_1 = variants; _a < variants_1.length; _a++) {
                    var expected = variants_1[_a];
                    var actual = filters_1.nextTypoPermutation(pattern, pos);
                    assert.equal(actual, expected);
                    pos += 1;
                }
                assert.equal(filters_1.nextTypoPermutation(pattern, pos), undefined);
            }
            assertTypos('abc', 'acb');
            assertTypos('foboar', 'fbooar', 'foobar', 'fobaor', 'fobora');
        });
        test('fuzzyScoreGraceful', function () {
            assertMatches('tkb', 'the_black_knight', '^the_^black_^knight', filters_1.fuzzyScoreGraceful);
            assertMatches('tkbk', 'the_black_knight', '^the_^blac^k_^knight', filters_1.fuzzyScoreGraceful);
            assertMatches('tkkb', 'the_black_knight', undefined, filters_1.fuzzyScoreGraceful);
            assertMatches('tkb', 'no_match', undefined, filters_1.fuzzyScoreGraceful);
        });
    });
});
