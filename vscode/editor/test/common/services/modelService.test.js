define(["require", "exports", "assert", "vs/platform/configuration/test/common/testConfigurationService", "vs/editor/common/services/modelServiceImpl", "vs/base/common/uri", "vs/base/common/platform", "vs/editor/common/editorCommon", "vs/editor/common/model/model", "vs/editor/common/model/textSource", "vs/editor/common/core/editOperation", "vs/editor/common/core/range", "vs/editor/common/core/stringBuilder"], function (require, exports, assert, testConfigurationService_1, modelServiceImpl_1, uri_1, platform, editorCommon_1, model_1, textSource_1, editOperation_1, range_1, stringBuilder_1) {
    /*---------------------------------------------------------------------------------------------
    ...
    suite('ModelService', function () {
        ...
        test('EOL setting respected depending on root', function () {
            ...
            assert.equal(model1.getOptions().defaultEOL, editorCommon_1.DefaultEndOfLine.LF);
            assert.equal(model2.getOptions().defaultEOL, editorCommon_1.DefaultEndOfLine.CRLF);
            assert.equal(model3.getOptions().defaultEOL, editorCommon_1.DefaultEndOfLine.LF);
        });
        test('_computeEdits first line changed', function () {
            ...
            var textSource = textSource_1.TextSource.fromString([
                'This is line One',
                'and this is line number two',
                'it is followed by #3',
                'and finished with the fourth.',
            ].join('\n'), editorCommon_1.DefaultEndOfLine.LF);
            var actual = modelServiceImpl_1.ModelServiceImpl._computeEdits(model, textSource);
    ...
    function assertComputeEdits(lines1, lines2) {
        var model = model_1.Model.createFromString(lines1.join('\n'));
        var textSource = textSource_1.TextSource.fromString(lines2.join('\n'), editorCommon_1.DefaultEndOfLine.LF);
        // compute required edits
        // let start = Date.now();
        var edits = modelServiceImpl_1.ModelServiceImpl._computeEdits(model, textSource);
