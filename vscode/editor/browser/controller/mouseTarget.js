    ...
    var MouseTargetFactory = (function () {
        ...
        MouseTargetFactory._hitTestMargin = function (ctx, request) {
            if (request.isInMarginArea) {
                ...
                if (offset <= ctx.layoutInfo.glyphMarginWidth) {
                    // On the glyph margin
                    return request.fulfill(editorBrowser_1.MouseTargetType.GUTTER_GLYPH_MARGIN, pos, res.range, res.isAfterLines);
                }
                offset -= ctx.layoutInfo.glyphMarginWidth;
                if (offset <= ctx.layoutInfo.lineNumbersWidth) {
                    // On the line numbers
                    return request.fulfill(editorBrowser_1.MouseTargetType.GUTTER_LINE_NUMBERS, pos, res.range, res.isAfterLines);
                }
                offset -= ctx.layoutInfo.lineNumbersWidth;
                // On the line decorations
                return request.fulfill(editorBrowser_1.MouseTargetType.GUTTER_LINE_DECORATIONS, pos, res.range, res.isAfterLines);
            }
            return null;
        };
        MouseTargetFactory._hitTestViewLines = function (ctx, request, domHitTestExecuted) {
            ...
            if (ctx.isAfterLines(request.mouseVerticalOffset)) {
                ...
                return request.fulfill(editorBrowser_1.MouseTargetType.CONTENT_EMPTY, new position_1.Position(lineCount, maxLineColumn));
            }
            if (domHitTestExecuted) {
        ...
        MouseTargetFactory.createMouseTargetFromHitTestPosition = function (ctx, request, lineNumber, column) {
            ...
            if (request.mouseContentHorizontalOffset > lineWidth) {
                if (browser.isEdge && pos.column === 1) {
                    ...
                    return request.fulfill(editorBrowser_1.MouseTargetType.CONTENT_EMPTY, new position_1.Position(lineNumber, ctx.model.getLineMaxColumn(lineNumber)));
                }
                return request.fulfill(editorBrowser_1.MouseTargetType.CONTENT_EMPTY, pos);
            }
