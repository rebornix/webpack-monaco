define(["require", "exports", "vs/editor/common/core/position", "vs/base/common/strings", "vs/editor/common/model/textModel", "vs/editor/common/core/selection", "vs/editor/common/core/range", "vs/editor/common/modes/languageConfigurationRegistry", "vs/base/common/errors"], function (require, exports, position_1, strings, textModel_1, selection_1, range_1, languageConfigurationRegistry_1, errors_1) {
    ...
    var CursorConfiguration = (function () {
        function CursorConfiguration(languageIdentifier, oneIndent, modelOptions, configuration) {
            ...
            this.electricChars = {};
            var electricChars = CursorConfiguration._getElectricCharacters(languageIdentifier);
            if (electricChars) {
    ...
    var CursorContext = (function () {
        ...
        CursorContext.prototype.getCompletelyVisibleModelRangeAtScrollTop = function (scrollTop) {
            var viewRange = this.viewModel.getCompletelyVisibleViewRangeAtScrollTop(scrollTop);
            return this.viewModel.coordinatesConverter.convertViewRangeToModelRange(viewRange);
        };
    ...
    var CursorState = (function () {
        ...
        CursorState.ensureInEditableRange = function (context, states) {
            var model = context.model;
            if (!model.hasEditableRange()) {
                return states;
            }
            var modelEditableRange = model.getEditableRange();
            var viewEditableRange = context.convertModelRangeToViewRange(modelEditableRange);
            var result = [];
            for (var i = 0, len = states.length; i < len; i++) {
                var state = states[i];
                if (state.modelState) {
                    var newModelState = CursorState._ensureInEditableRange(state.modelState, modelEditableRange);
                    result[i] = newModelState ? CursorState.fromModelState(newModelState) : state;
                }
                else {
                    var newViewState = CursorState._ensureInEditableRange(state.viewState, viewEditableRange);
                    result[i] = newViewState ? CursorState.fromViewState(newViewState) : state;
                }
            }
            return result;
        };
        CursorState._ensureInEditableRange = function (state, editableRange) {
            ...
            if (position.lineNumber < editableRange.startLineNumber || (position.lineNumber === editableRange.startLineNumber && position.column < editableRange.startColumn)) {
                return new SingleCursorState(state.selectionStart, state.selectionStartLeftoverVisibleColumns, new position_1.Position(editableRange.startLineNumber, editableRange.startColumn), 0);
            }
            if (position.lineNumber > editableRange.endLineNumber || (position.lineNumber === editableRange.endLineNumber && position.column > editableRange.endColumn)) {
                return new SingleCursorState(state.selectionStart, state.selectionStartLeftoverVisibleColumns, new position_1.Position(editableRange.endLineNumber, editableRange.endColumn), 0);
            }
            return null;
        };
    ...
    var EditOperationResult = (function () {
        function EditOperationResult(commands, opts) {
