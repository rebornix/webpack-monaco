define(["require", "exports", "vs/base/common/errors", "vs/editor/common/commands/replaceCommand", "vs/editor/common/controller/cursorCommon", "vs/editor/common/core/range", "vs/base/common/strings", "vs/editor/common/commands/shiftCommand", "vs/editor/common/modes/languageConfigurationRegistry", "vs/editor/common/modes/languageConfiguration", "vs/editor/common/commands/surroundSelectionCommand", "vs/editor/common/controller/wordCharacterClassifier"], function (require, exports, errors_1, replaceCommand_1, cursorCommon_1, range_1, strings, shiftCommand_1, languageConfigurationRegistry_1, languageConfiguration_1, surroundSelectionCommand_1, wordCharacterClassifier_1) {
    ...
    var TypeOperations = (function () {
        ...
        TypeOperations._distributePasteToCursors = function (selections, pasteOnNewLine, text) {
            if (pasteOnNewLine) {
                return null;
            }
            ...
            var pastePieces = text.split(/\r\n|\r|\n/);
            if (pastePieces.length !== selections.length) {
                return null;
            }
            return pastePieces;
        };
        TypeOperations.paste = function (config, model, selections, pasteOnNewLine, text) {
            var distributedPaste = this._distributePasteToCursors(selections, pasteOnNewLine, text);
            if (distributedPaste) {
        ...
        TypeOperations._goodIndentForLine = function (config, model, lineNumber) {
            var action;
            var indentation;
            var expectedIndentAction = languageConfigurationRegistry_1.LanguageConfigurationRegistry.getInheritIndentForLine(model, lineNumber, false);
        ...
        TypeOperations.replacePreviousChar = function (config, model, selections, txt, replaceCharCnt) {
            var commands = [];
            for (var i = 0, len = selections.length; i < len; i++) {
                var selection = selections[i];
                if (!selection.isEmpty()) {
        ...
        TypeOperations._runAutoClosingCloseCharType = function (config, model, selections, ch) {
            var commands = [];
            for (var i = 0, len = selections.length; i < len; i++) {
                var selection = selections[i];
                var position = selection.getPosition();
        ...
        TypeOperations._runAutoClosingOpenCharType = function (config, model, selections, ch) {
            var commands = [];
            for (var i = 0, len = selections.length; i < len; i++) {
                var selection = selections[i];
        ...
        TypeOperations._runSurroundSelectionType = function (config, model, selections, ch) {
            var commands = [];
            for (var i = 0, len = selections.length; i < len; i++) {
                var selection = selections[i];
        ...
        TypeOperations._typeInterceptorElectricChar = function (config, model, selection, ch) {
        ...
        TypeOperations.typeWithInterceptors = function (config, model, selections, ch) {
            if (ch === '\n') {
                var commands = [];
                for (var i = 0, len = selections.length; i < len; i++) {
            ...
            if (this._isAutoIndentType(config, model, selections)) {
                var indentCommand = this._runAutoIndentType(config, model, selections[0], ch);
                if (indentCommand) {
                    return new cursorCommon_1.EditOperationResult([indentCommand], {
                        shouldPushStackElementBefore: true,
                        shouldPushStackElementAfter: false,
                    });
                }
            ...
            if (this._isTypeInterceptorElectricChar(config, model, selections)) {
                var r = this._typeInterceptorElectricChar(config, model, selections[0], ch);
                if (r) {
                    return r;
                }
            }
        ...
        TypeOperations.typeWithoutInterceptors = function (config, model, selections, str) {
            var commands = [];
            for (var i = 0, len = selections.length; i < len; i++) {
