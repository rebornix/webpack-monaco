    ...
    var BackupMainService = (function () {
        ...
        BackupMainService.prototype.validateBackupWorkspaces = function (backups) {
            ...
            workspaceAndFolders.forEach(function (workspaceOrFolder) {
                ...
                if (hasBackups && !missingWorkspace && !workspaces_1.isSingleFolderWorkspaceIdentifier(workspaceId) && workspaceId.id !== _this.workspacesService.getWorkspaceId(workspacePath)) {
                    staleBackupWorkspaces.push({ workspaceIdentifier: workspaceId, backupPath: backupPath, target: workspaceOrFolder.target });
                    var identifier = { id: _this.workspacesService.getWorkspaceId(workspacePath), configPath: workspacePath };
                    _this.pushBackupPathsSync(identifier, _this.backups.rootWorkspaces);
                    var newWorkspaceBackupPath = path.join(_this.backupHome, identifier.id);
                    try {
                        fs.renameSync(backupPath, newWorkspaceBackupPath);
                    }
                    catch (ex) {
                        _this.logService.error("Backup: Could not rename backup folder for legacy workspace: " + ex.toString());
                        _this.removeBackupPathSync(identifier, _this.backups.rootWorkspaces);
                    }
                }
                ...
                if (!hasBackups || missingWorkspace) {
                    ...
                    if (missingWorkspace) {
                        ...
                        var newEmptyWindowBackupPath = path.join(_this.backupHome, identifier);
                        try {
